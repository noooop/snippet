#!/usr/bin/env python3
"""
Integrated benchmark: compare GPU‑to‑CPU block‑transfer bandwidth for two scenarios:
  1. zmq         – GPU data copied to CPU, then sent via ZMQ (server writes to CPU buffer).
  2. zmq+shm     – GPU data written directly to shared memory via optimized kernel;
                   server is notified via ZMQ and reads from shared memory,
                   optionally performing an extra CPU copy (client‑controlled flag).
...
# ---------------------------------------------------------------------------
# Shared memory helpers (used by zmq+shm scenario)
# ---------------------------------------------------------------------------
...
def shared_tensor_pair(total_bytes: int):
    ...
    Used for shared memory benchmarks (zmq+shm).
    """
...
def blocks_batch(src: torch.Tensor, dst: torch.Tensor, block_size: int):
    """
    Copy data from a GPU tensor `src` to a CPU tensor `dst` using a fast
    CUDA kernel (vLLM swap_blocks_batch). Falls back to a normal copy if
    the custom op is not available.
    """
    if ops is None:
        dst.copy_(src)
        return
...
class ZmqServerProc:
    ...
    mode: 'zmq' – receives data blocks directly (no extra copy on server).
          'zmq+shm' – receives index, block size and a copy flag;
                      reads from shared memory (optionally copies to local buffer).
...
def _zmq_server_worker(conn, stop_event, mode, shm_name, total_bytes):
    """
    ZMQ server that handles two GPU‑to‑CPU protocols:
      - 'zmq': receive data block (already copied to CPU by client),
               write directly into a CPU buffer.
      - 'zmq+shm': receive index (8 bytes), block size (4 bytes), and a copy flag (1 byte).
                   Read the block from shared memory (written by GPU via kernel).
                   If copy flag is True, perform an additional CPU copy to a local buffer.
                   If False, simply acknowledge without copying.
    The server runs once and handles all requests until stop_event is set.
    """
    context = zmq.Context()
    ...
    dtype = torch.uint8
    # Destination CPU buffer where final data is collected
    ...
    while not stop_event.is_set():
        ...
        if mode == "zmq":
            ...
            dst_np[start: start+expected_size] = payload.buffer
        else:  # 'zmq+shm'
            ...
            # Data was placed into shared memory by the GPU kernel on the client side
            payload = arr[start : start + expected_size]
            if copy_data:
                # Perform an explicit CPU copy (e.g., for measurement purposes)
                payload = payload.copy()
            assert expected_size == payload.nbytes
            dst_np[start: start+expected_size] = payload
...
class ZmqClient:
    ...
    Provides two request methods:
      - request_data(payload)        for 'zmq' mode (GPU data already on CPU)
    ...
    def request_index(self, idx: int, block_bytes: int, copy_data: bool = True):
        ...
        The server will compute the byte offset as idx * block_bytes.
        If copy_data is True, the server will copy the data; otherwise, it will not.
        """
        sock = self._pool.pop() if self._pool else self._init_sock()
...
def run_zmq(total_bytes, block_sizes, n_iters):
    """
    GPU‑to‑CPU transfer using ZMQ:
    The client copies a GPU block to CPU, then sends it via ZMQ.
    The server writes the received data into a CPU buffer.
    """
    if not HAS_ZMQ:
    ...
    for bs_bytes in block_sizes:
        ...
        for s_idx, d_idx, in zip(src_indices, dst_indices):
            # Copy block from GPU to CPU, then send
...
def run_zmq_shm(total_bytes, block_sizes, n_iters, copy_data=True):
    """
    GPU‑to‑CPU transfer using ZMQ + shared memory:
    The client uses an optimized GPU kernel (`blocks_batch`) to write a block
    directly into shared memory. Then it sends the block index and size to the
    server via ZMQ. The server reads the data from shared memory and optionally
    performs an additional CPU copy (controlled by `copy_data`).
    """
    if not HAS_ZMQ:
    ...
    with shared_tensor_pair(total_bytes) as (_, dst, shm_src, shm_dst):
        ...
        # Initialize the CPU-side shared memory destination
        dst[:] = torch.randn(total_bytes // 4, dtype=torch.float32).view(torch.uint8)
        print(f"Allocated shared memory segment of {format_size(total_bytes)} for zmq+shm")

        # Start server once; it reads from the same shared memory that the GPU writes to
        # (using `shm_dst` name as that is the destination buffer).
        server = ZmqServerProc(
            stop_event, mode="zmq+shm", total_bytes=total_bytes, shm_name=shm_dst.name
        )
        server.start()
        ...
        for bs_bytes in block_sizes:
            ...
            for i, j in zip(src_indices, dst_indices):
                # GPU -> shared memory via optimized kernel
                blocks_batch(src_view[i], dst_view[j], bs_bytes)
                # Notify server (read from shared memory)
...
def print_results_table(block_sizes: list, results: dict):
    ...
    print("### GPU‑to‑CPU Bandwidth Results (GiB/s)")
    print("| " + " | ".join(header) + " |")
...
def plot_results(block_sizes: list, results: dict, output_file: str = None):
    ...
    ax.set_title(f"GPU‑to‑CPU Block Transfer Bandwidth\n({sys_info})")
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark GPU‑to‑CPU block transfer using ZMQ and ZMQ+shared memory."
    )
    parser.add_argument(
        "--size",
        type=int,
    ...
    parser.add_argument(
        ...
        choices=["zmq", "zmq+shm"],
        default=["zmq", "zmq+shm"],
        help="Which benchmarks to run. Default: both zmq and zmq+shm.",
    )
    parser.add_argument(
        "--no-plot",
        action="store_true",
