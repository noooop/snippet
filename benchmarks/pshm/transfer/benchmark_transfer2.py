#!/usr/bin/env python3
"""
Integrated benchmark: compare CPU block‑copy bandwidth for four scenarios:
  ...
  3. zmq         – send data blocks via ZMQ (server does not copy, zero‑copy idea)
  4. zmq+shm     – send block indices and size via ZMQ; server reads from shared memory
                   and optionally copies (client‑controlled flag).
...
def _zmq_server_worker(conn, stop_event, mode, shm_name, total_bytes):
    ...
    ZMQ server that handles two protocols:
      - 'zmq': receive data block (no extra copy on server)
      - 'zmq+shm': receive index (8 bytes), block size (4 bytes), and a copy flag (1 byte).
                   If copy flag is True, read from shared memory and copy into a local buffer.
...
class ZmqClient:
    ...
    def request_data(self, payload: memoryview):
...
def run_zmq(total_bytes, block_sizes, n_iters):
    """ZMQ transfer without server‑side copy (zero‑copy idea)."""
    if not HAS_ZMQ:
    ...
    src = torch.randn(total_bytes // 4, dtype=torch.float32, device="cpu").view(dtype)
    src_np = src.numpy()
    print(f"Allocated {format_size(total_bytes)} for zmq source")
...
def run_zmq_shm(total_bytes, block_sizes, n_iters, copy_data=True):
    """
    ZMQ + shared memory: send indices and block sizes; server reads from shared memory
    and optionally copies. Server is started once and handles all block sizes.
    """
    if not HAS_ZMQ:
...
def plot_results(block_sizes: list, results: dict, output_file: str = None):
    ...
    ax.set_title(f"CPU Block Copy Bandwidth (Four Scenarios)\n({sys_info})")
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark CPU block copy: memcpy, shm, zmq, zmq+shm (with/without copy)."
    )
    parser.add_argument(
        "--size",
        type=int,
    ...
    parser.add_argument(
        "--bench",
        nargs="+",
        choices=["memcpy", "shm", "zmq", "zmq+shm"],
        default=["memcpy", "shm", "zmq", "zmq+shm"],
