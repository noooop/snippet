...
@torch.inference_mode()
def test_prefill_decode(seqlen_k):
    n_decode_tokens = 4
    ...
    if n_prefill_tokens > 0:
        ...
        seqlens_k = [seqlen_k] * n_decode_tokens
...
for i in range(2, 12):
    test_prefill_decode(2**i)


@torch.inference_mode()
def test_decode_prefill(seqlen_k):
    n_decode_tokens = 4
    n_prefill_tokens = 128

    decode_q = torch.randn(n_decode_tokens, num_heads, head_size, dtype=torch.bfloat16, device="cuda")
    decode_key_cache = torch.randn(n_decode_tokens * seqlen_k, num_heads, head_size, dtype=torch.bfloat16,
                                   device="cuda")
    decode_value_cache = torch.randn(n_decode_tokens * seqlen_k, num_heads, head_size, dtype=torch.bfloat16,
                                     device="cuda")
    decode_output = torch.randn(n_decode_tokens, num_heads, head_size, dtype=torch.bfloat16, device="cuda")
